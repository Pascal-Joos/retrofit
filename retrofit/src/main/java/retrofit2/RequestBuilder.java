...
final class RequestBuilder {
  ...
  private static String canonicalizeForPath(@Nullable String input, boolean alreadyEncoded) {
    if (input == null) {
      throw new IllegalArgumentException("@Path parameter value must not be null.");
    }
    int codePoint;
  ...
  void addFormField(String name, String value, boolean encoded) {
    if (encoded) {
      java.util.Objects.requireNonNull(formBuilder).addEncoded(name, value);
    } else {
      java.util.Objects.requireNonNull(formBuilder).add(name, value);
    }
  }
  ...
  void addPart(Headers headers, @Nullable RequestBody body) {
    java.util.Objects.requireNonNull(multipartBuilder).addPart(headers, body);
  }

  @SuppressWarnings("ConstantConditions") // Only called when isMultipart was true.
  void addPart(MultipartBody.Part part) {
    java.util.Objects.requireNonNull(multipartBuilder).addPart(part);
  }
